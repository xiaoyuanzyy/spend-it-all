...
async function syncReign(champion) {
  const reignsCol = db.collection('spendItAll_reigns');
...
exports.main = async () => {
  try {
    ...
    const profiles = await fetchAll(db.collection('spendItAll_profiles'));
    const bills = await fetchAll(db.collection('spendItAll_bills'), { deleted: _.neq(true) });
